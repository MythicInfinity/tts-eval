...
IMAGE_NAME="${IMAGE_NAME:-tts-eval-audiobox}"
...
BATCH_SIZE="${AUDIOBOX_BATCH_SIZE:-16}"
DEVICE="${AUDIOBOX_DEVICE:-cuda}"
...
TIMESTAMP="${3:-}"
OUTPUT_DIR="/app/data/evals/audiobox"
...
docker_args=(
  ...
  "${IMAGE_NAME}"
  python
  /app/eval/runners/audiobox/run_inner.py
  --inputs
  /inputs
  --refs
  /refs
