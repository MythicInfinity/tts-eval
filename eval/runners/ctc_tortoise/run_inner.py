...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run the Tortoise-backed CTC closeness metric over model directories.")
...
def main() -> int:
    ...
    runtime = load_tortoise_ctc_runtime(args.device)
    models = iter_models(args.inputs)
    log_runner_start("ctc_tortoise", run_timestamp_utc, len(models))

    for index, model_input in enumerate(models, start=1):
        log_model_progress("ctc_tortoise", model_input.model, index, len(models))
        ...
        metadata_payload["transformers_version"] = runtime.transformers.__version__
        metadata_payload["hf_model"] = runtime.model_id
        metadata_payload["hf_vocab"] = runtime.vocab_repo_id
        metadata_payload["device"] = runtime.device
        metadata_payload["blank_id"] = runtime.blank_id
        metadata_payload["ctc_zero_infinity"] = runtime.ctc_zero_infinity
        ...
        log_model_summary("ctc_tortoise", summary_payload)

    log_runner_end("ctc_tortoise", len(models))
