...
IMAGE_NAME="${IMAGE_NAME:-tts-eval-nisqa}"
...
BATCH_SIZE="${NISQA_BATCH_SIZE:-8}"
DEVICE="${NISQA_DEVICE:-auto}"
...
TIMESTAMP="${3:-}"
OUTPUT_DIR="/app/data/evals/nisqa"
...
docker_args=(
  ...
  "${IMAGE_NAME}"
  python
  /app/eval/runners/nisqa/run_inner.py
  --inputs
  /inputs
  --refs
  /refs
