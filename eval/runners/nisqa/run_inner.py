...
from tts_eval.nisqa import build_metadata_payload, build_summary_payload, evaluate_model, load_nisqa_runtime
from tts_eval.progress import log_model_progress, log_model_summary, log_runner_end, log_runner_start


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run NISQA MOS over model directories.")
...
def main() -> int:
    ...
    runtime = load_nisqa_runtime(execution_device=execution_device)
    models = iter_models(args.inputs)
    log_runner_start("nisqa", run_timestamp_utc, len(models))

    for index, model_input in enumerate(models, start=1):
        log_model_progress("nisqa", model_input.model, index, len(models))
        ...
        log_model_summary("nisqa", summary_payload)

    log_runner_end("nisqa", len(models))
